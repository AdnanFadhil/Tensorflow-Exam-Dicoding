...
def solution_B4():
    ...
    train_x = tokenizer.texts_to_sequences(features_train)
    train_x = pad_sequences(train_x, maxlen=max_length, padding=padding_type, truncating=trunc_type)

    test_x = tokenizer.texts_to_sequences(features_test)
    test_x = pad_sequences(test_x, maxlen=max_length, padding=padding_type, truncating=trunc_type)

    label_tokenizer = Tokenizer()
    label_tokenizer.fit_on_texts(labels)

    train_y = np.array(label_tokenizer.texts_to_sequences(labels_train))
    test_y = np.array(label_tokenizer.texts_to_sequences(labels_test))
